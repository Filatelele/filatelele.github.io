				...
				for(let res_load of data.resource_loads) {
					...
					if(res_load.path) {
						...
						if(!res.data) {
							...
							if(res_load.path.startsWith(paintings_pattern) && this.rev_data?.repo == "ChaoticOnyx/OnyxBay") {
								res.load_url = "https://cdn.yogstation.net/paintings/" + res_load.path.substring(paintings_pattern.length);
							} else {
								res.load_url = `https://cdn.jsdelivr.net/gh/${this.rev_data?.repo || "ChaoticOnyx/OnyxBay"}@${(this.rev_data?.commit || "master")}/${res_load.path}`;
							}
						}
					}
					res.update();
